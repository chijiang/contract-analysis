...
export async function POST(req: NextRequest) {
  ...
  const contractIds = normalizeIds(body?.contractIds)

  if (contractIds.length === 0) {
    return new Response(JSON.stringify({ message: "请选择至少一个合同再导出" }), {
      status: 400,
  ...
  if (contracts.length === 0) {
    return new Response(JSON.stringify({ message: "未找到匹配的合同记录" }), {
      status: 404,
      headers: { "Content-Type": "application/json" },
    })
  }
  ...
  for (const contract of contracts) {
    ...
    const selectedTemplateIds = contract.analysis
      ? normalizeTemplateIds(contract.analysis.selectedTemplateIds)
      : null
    ...
    const formatDeviceList = (devices: ReturnType<typeof parseServiceInfoSnapshotPayload>["onsiteSla"][number]["devices"]) =>
      devices
        .map((device) => {
          const parts = [
            device.deviceName ?? "",
            device.deviceModel ?? "",
            device.geHostSystemNumber ?? "",
      ...
      snapshot.keySpareParts.forEach((item) => {
        ...
        if (item.tubes.length) {
          extra.push(
            `球管：${item.tubes
              .map((tube) =>
                [tube.deviceModel, tube.geHostSystemNumber, tube.xrTubeId].filter(Boolean).join("/"),
              )
              .filter(Boolean)
              .join(" | ")}`,
          )
